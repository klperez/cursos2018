...
Veamos los datos utilizados por _Francis Galton en 1885_. Galton fue un estadístico que inventó el término y los conceptos de regresión y correlación, fundó la revista Biometrika, además de ser el primo de _Charles Darwin_.

```{r, message=FALSE, warning=FALSE, echo=TRUE}
library(UsingR); library(tidyr); library(dplyr); data(galton)
glimpse(galton)
```

## Galton's Data - Ajuste
...
ui <- fluidPage(
  fluidRow(
    column(4,
           wellPanel(
             sliderInput(inputId = "beta", label = "Ajuste del Beta:",
              min = 0.6, max = 1.2, value = 1, step = 0.02)
           )
    ),
    column(8,
...
server <- function(input, output) {
    ...
    mse <- mean( (y - beta * x) ^2 )
    g <- g + theme_minimal() + ggtitle(paste("beta = ", beta, "mse = ", round(mse, 3)))
    g
        }
        myPlot(input$beta)
    })
